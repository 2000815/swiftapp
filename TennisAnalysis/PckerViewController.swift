//
//  PckerViewController.swift
//  TennisAnalysis
//
//  Created by 市田圭司 on 2021/12/08.
//
...
class PckerViewController: UIViewController,UIPickerViewDelegate, UIPickerViewDataSource {
    
    
    var pickerTag:Int = 1
    var postId : String?
    var SelectPost :NCMBObject?
    ...
    var selectpickerView : UIPickerView = UIPickerView()
    ...
    override func viewDidLoad() {
        ...
        additionButton.layer.cornerRadius = 15.0
    ...
    func picker(hav: UITextField){
       ...
       // hav.inputAccessoryView = toolbar
        // 5. デフォルト設定
...
//        pickerView1.selectRow(4, inComponent: 0, animated: false)
        ...
        @objc func done() {
            print(pickerTag)
            if selectpickerView.selectedRow(inComponent: 0) == 0{
                return
            }else if(pickerTag == 1){
    ...
    //// 決定ボタンのアクション指定
    @IBAction func save(){
        let obj = NCMBObject(className: "points")
        obj?.setObject(MypointTextFild.text, forKey: "mypoint")
        obj?.setObject(enemypointTextFild.text, forKey: "enemypoint")
        obj?.setObject(MygameTextFild.text, forKey: "Mygame")
        obj?.setObject(enemygameTextFild.text, forKey: "enemygame")
        obj?.setObject(foaTextFild.text, forKey: "foa")
        obj?.setObject(shotTextFild.text, forKey: "shot")
        obj?.setObject(resultTextFild.text, forKey: "result")
        obj?.setObject(postId, forKey: "postId")
        obj?.setObject(memoTextView.text, forKey: "memo")
        
        if MypointTextFild.text!.count == 0 || enemypointTextFild.text!.count == 0 || MygameTextFild.text!.count == 0 || enemygameTextFild.text!.count == 0 || foaTextFild.text!.count == 0 || shotTextFild.text!.count == 0 || resultTextFild.text!.count == 0 || postId == nil{
